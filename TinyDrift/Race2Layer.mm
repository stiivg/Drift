...
@implementation Race2Layer

//SJG TODO link failed when same name as GameplayLayer
CCParticleSystem * _drift3_emitter;
CCParticleSystem * _turbo3_emitter;
...
-(void)setupEmitters {
    //Normal Drift
    _drift3_emitter = [CCParticleDrift node];
    _drift3_emitter.emissionRate = 0.0;
    [_terrain addChild: _drift3_emitter];
    [_drift3_emitter setPosition:ccp(_car.position.x, _car.position.y)];
    
    //Turbo drift
    _turbo3_emitter = [CCParticleTurbo node];
    _turbo3_emitter.emissionRate = 0.0;
    [_terrain addChild: _turbo3_emitter];
    [_turbo3_emitter setPosition:ccp(_car.position.x, _car.position.y)];
}
